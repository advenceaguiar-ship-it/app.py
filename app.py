...
# Instala automaticamente o gTTS se faltar
try:
...
# --- CAMINHO DO EXCEL ---
arquivo_excel = r"C:\Users\eidry\OneDrive\Documentos\I.A\relatorios_manutencao.xlsx"
...
filtro_setor = st.sidebar.text_input("Filtrar por Setor (Ex: Fundo, Usinagem):")
...
if raw_key:
    ...
    if st.session_state.etapa == 1:
        ...
        # Trava de segurança: só executa se houver áudio E se ainda não tiver processado este envio
        if audio_file is not None and not st.session_state.ja_processou:
            st.session_state.ja_processou = True # Ativa a trava imediatamente
    ...
    # ==========================================
        ...
        st.audio("pergunta_ia.mp3", format="audio/mp3", autoplay=True)
        
        audio_resposta = st.audio_input("Grave sua resposta para concluir:")

        # Trava de segurança para a etapa 2 também
    ...
    if os.path.exists(arquivo_excel):
        try:
            ...
            if filtro_tipo != "Todos":
                df_filtrado = df_filtrado[df_filtrado["Tipo de Serviço"].str.contains(filtro_tipo, case=False, na=False)]
                
            if filtro_setor:
                df_filtrado = df_filtrado[df_filtrado["Setor / Área"].str.contains(filtro_setor, case=False, na=False)]
